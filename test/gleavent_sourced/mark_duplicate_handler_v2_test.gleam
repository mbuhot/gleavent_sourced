...
import gleavent_sourced/customer_support/ticket_commands.{
  MarkDuplicateCommand, BusinessRuleViolation,
}
...
pub fn successful_duplicate_marking_test() {
  test_runner.txn(fn(db) {
    ...
    let command = MarkDuplicateCommand(
      "T-101",  // duplicate_ticket_id
      "T-100",  // original_ticket_id
      "2024-01-01T15:00:00Z",  // marked_at
    )
    let handler = mark_duplicate_handler_v2.create_mark_duplicate_handler_v2(command)
    ...
    let assert [TicketMarkedDuplicate("T-101", "T-100", "2024-01-01T15:00:00Z")] = events
  })
}
...
pub fn mark_duplicate_with_missing_original_fails_test() {
  test_runner.txn(fn(db) {
    ...
    let command = MarkDuplicateCommand(
      "T-102",  // duplicate_ticket_id (exists)
      "T-999",  // original_ticket_id (doesn't exist)
...
pub fn mark_duplicate_with_missing_duplicate_ticket_fails_test() {
  test_runner.txn(fn(db) {
    ...
    let command = MarkDuplicateCommand(
      "T-999",  // duplicate_ticket_id (doesn't exist)
      "T-103",  // original_ticket_id (exists)
      "2024-01-01T15:00:00Z",
    )
    let handler = mark_duplicate_handler_v2.create_mark_duplicate_handler_v2(command)
    let metadata = create_test_metadata()
...
pub fn mark_already_duplicate_ticket_fails_test() {
  test_runner.txn(fn(db) {
    ...
    let initial_events = [
      ...
      TicketMarkedDuplicate("T-105", "T-104", "2024-01-01T12:00:00Z"),  // T-105 already marked as duplicate of T-104
    ]
    let _ = setup_initial_events(db, initial_events)

    // Try to mark T-105 as duplicate of T-106 (but T-105 is already marked as duplicate)
    let command = MarkDuplicateCommand(
      "T-105",  // Already marked as duplicate
      "T-106",
      "2024-01-01T15:00:00Z",
    )
    let handler = mark_duplicate_handler_v2.create_mark_duplicate_handler_v2(command)
...
pub fn mark_duplicate_with_both_tickets_missing_fails_test() {
  test_runner.txn(fn(db) {
    ...
    let command = MarkDuplicateCommand(
      "T-998",  // duplicate_ticket_id (doesn't exist)
      "T-999",  // original_ticket_id (doesn't exist)
      "2024-01-01T15:00:00Z",
    )
    let handler = mark_duplicate_handler_v2.create_mark_duplicate_handler_v2(command)
...
pub fn mark_self_as_duplicate_fails_test() {
  test_runner.txn(fn(db) {
    ...
    let initial_events = [
      TicketOpened("T-107", "Self-referencing bug", "This will reference itself", "medium"),
    ]
    let _ = setup_initial_events(db, initial_events)

    // Try to mark T-107 as duplicate of itself (should be rejected)
    let command = MarkDuplicateCommand(
      "T-107",  // duplicate_ticket_id
      "T-107",  // original_ticket_id (same as duplicate)
      "2024-01-01T15:00:00Z",
    )
    let handler = mark_duplicate_handler_v2.create_mark_duplicate_handler_v2(command)
    let metadata = create_test_metadata()
...
pub fn mark_duplicate_creates_correct_event_structure_test() {
  test_runner.txn(fn(db) {
    ...
    let initial_events = [
      TicketOpened("T-200", "Bug in authentication", "Users can't log in", "critical"),
    ...
    let assert CommandAccepted(events) = result
    let assert [TicketMarkedDuplicate(duplicate_id, original_id, marked_at)] = events
