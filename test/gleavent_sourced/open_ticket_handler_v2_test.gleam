...
pub fn successful_ticket_creation_test() {
  test_runner.txn(fn(db) {
    ...
    let assert [TicketOpened("T-100", "Bug in payment system", "Payment processing fails intermittently", "high")] = events
  })
}
...
pub fn title_too_long_validation_fails_test() {
  test_runner.txn(fn(db) {
    ...
    let long_title = "This is a very long title that exceeds the maximum allowed length of 100 characters and should be rejected by validation logic"
    let command = OpenTicketCommand(
      "T-102",
      long_title,  // 125 characters - too long
      "Valid description",
      "critical",
      parent_ticket_id: None,
    )
...
pub fn invalid_priority_validation_fails_test() {
  test_runner.txn(fn(db) {
    ...
    let command = OpenTicketCommand(
      ...
      "Valid title",
      "Valid description",
      "urgent",  // Invalid priority - not in allowed list
      parent_ticket_id: None,
    )
    let handler = open_ticket_handler_v2.create_open_ticket_handler_v2()
    let metadata = create_test_metadata()
...
pub fn all_valid_priorities_succeed_test() {
  test_runner.txn(fn(db) {
    ...
    let _ = list.index_map(valid_priorities, fn(priority, index) {
      ...
      let assert Ok(result) = command_handler_v2.execute(db, handler, command, metadata)
      let assert CommandAccepted(events) = result
      let assert [TicketOpened(ticket_id_result, _, _, priority_result)] = events
